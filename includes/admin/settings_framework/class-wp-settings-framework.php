 ...
 * @subpackage Previously Bought Product Notifier For WooCommerce/WordPress/Settings
 * @since 3.0
 * @version 2.0
 * @author keesiemeijer
 */
if ( ! defined( 'WPINC' ) ) { die; }
class wc_previously_bought_product_notifier_Settings_Framework {
    private $page_hook = '';
    public $settings;
    ...
    function __construct($page_hook = '',$pageName = '') {
        ...
        if(empty($pageName)){
            $this->pageName = __('Previously Bought Product Notifier For WooCommerce',WC_PBPN_TXT);
        } else {
            wc_pbpn_add_vars('settings_page',$this->page_hook);
        }
    ...
    private function add_settings_pages(){
		$pages = array();
		$pages = apply_filters('wc_pbpn_settings_pages',$pages);
    ...
    private function add_settings_fields(){
        global $fields;
        $fields =  array();
		$fields = apply_filters('wc_pbpn_settings_fields',$fields);
                ...
                while($a < $c){
                    if(isset($s[$a]['validate_callback'])){
                        ...
                        $file = addslashes(PUGIN_SETTINGS.'validate-'.$s[$a]['id'].'.php');
                         $s[$a]['validate_callback'] = create_function('$fields', 'do_action("wc_pbpn_settings_validate",$fields); do_action("wc_pbpn_settings_validate_'.$s[$a]['id'].'",$fields);');
                    }
                    $a++;
                }
            }
    ...
    function admin_init(){ 
		$this->settings = new wc_previously_bought_product_notifier_WP_Settings();
		...
		$this->settings->init($pages, WC_PBPN_DB );
    }

    public function admin_page(){
		echo '<div class="wrap wc_pbpn_settings">';
		settings_errors();
