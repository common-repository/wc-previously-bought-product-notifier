 ...
 * @author keesiemeijer
 */
if ( !class_exists( 'wc_previously_bought_product_notifier_WP_Settings' ) ) {
	class wc_previously_bought_product_notifier_WP_Settings {
		...
		public function init( $pages, $page_hook = '' ) {
			...
			if ( !class_exists( 'wc_previously_bought_product_notifier_Settings_WP_Fields' ) ) {
				$this->debug  .= "Error: class wc_previously_bought_product_notifier_Settings_WP_Fields doesn't exist<br/>";
			}
			...
			$this->fields = new wc_previously_bought_product_notifier_Settings_WP_Fields( get_settings_errors() );
        ...
        function do_settings_sections( $page ) {
            ...
            if($section_count > 1){
                echo '<ul class="subsubsub wc_pbpn_settings_submenu">';
            ...
            foreach ( (array) $wp_settings_sections[$page] as $section ) {
                if($section_count > 1){ echo '<div id="settings_'.$section['id'].'" class="hidden wc_pbpn_settings_content">'; }
                    if ( $section['title'] )
