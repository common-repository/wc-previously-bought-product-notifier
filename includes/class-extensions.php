...
class wc_previously_bought_product_notifier_Addons {
    
    public function __construct() {
        add_action( 'wp_ajax_wc_pbpn_get_addons_html',array($this,'list_addons_ajax'));
    	add_action(WC_PBPN_DB.'_form_fields',array($this,'list_addons'),10,2);
		add_action( 'wp_ajax_wc_pbpn_activate_addon',array($this,'activate_plugin'));
		add_action( 'wp_ajax_wc_pbpn_deactivate_addon',array($this,'deactivate_plugin'));
    }	
	
	public function deactivate_plugin(){
		$status = $this->addon_actions('deactivate');
		if($status === 'invalidcode'){
			wp_send_json_error(array('msg' => '<span class="wc_pbpn_ajax_error">'.__('Unable to process you request. please try again later',WC_PBPN_TXT).'</span>'));
		} else if($status === 'verifyfailed'){
			wp_send_json_error(array('msg' => '<span class="wc_pbpn_ajax_ajaxerror">'.__('Unable To De-Activate Addon. Please Try Again Later',WC_PBPN_TXT).'</span>'));		
		} else if($status === true){
			wp_send_json_success(array('msg' => '<span class="wc_pbpn_ajax_success">'.__('Addon De-Activated',WC_PBPN_TXT).'</span>'));		
		} else if($status === 'alreadyactive'){
			wp_send_json_success(array('msg' => '<span class="wc_pbpn_ajax_success">'.__('Addon Already De-Activated',WC_PBPN_TXT).'</span>'));		
		} else {
			wp_send_json_error(array('msg' => '<span class="wc_pbpn_ajax_error">'.__('Unable To De-Activate Addon. Please Try Again Later',WC_PBPN_TXT).'</span>'));		
		}
		wp_die();  
	}
	...
	public function activate_plugin(){
		...
		if($status === 'invalidcode'){
			...
			wp_send_json_error(array('msg' => '<span class="wc_pbpn_ajax_ajaxerror">'.__('Unable To Activate Addon. Please Try Again Later',WC_PBPN_TXT).'</span>'));		
		} else if($status === true){
			wp_send_json_success(array('msg' => '<span class="wc_pbpn_ajax_success">'.__('Addon Activated',WC_PBPN_TXT).'</span>'));		
		} else if($status === 'alreadyactive'){
			wp_send_json_success(array('msg' => '<span class="wc_pbpn_ajax_success">'.__('Addon Already Activated',WC_PBPN_TXT).'</span>'));		
		} else {
			wp_send_json_error(array('msg' => '<span class="wc_pbpn_ajax_error">'.__('Unable To Activate Addon. Please Try Again Later',WC_PBPN_TXT).'</span>'));		
		}
		wp_die();  
	}
	
	public function addon_actions($action = 'activate'){
		if(! isset($_REQUEST['wc_pbpn_security_code'])){ return 'invalidcode'; }
		$nonce_action = 'wc_pbpn_'.$action.'_addon';
		$verify = wp_verify_nonce( $_REQUEST['wc_pbpn_security_code'], $nonce_action );
		if(!$verify){return 'verifyfailed';}
		$function_call = 'wc_pbpn_'.$action.'_addon';
	...
	public function search_and_get_addons(){
		$search_dirs = apply_filters('wc_pbpn_addons_dir',array()); 
		$addons_others = array();
		$internal_addons = $this->get_plugins(WC_PBPN_ADDON);
	...
	public function generate_view(){
		...
		include(WC_PBPN_ADMIN.'views/addons-header.php');
		foreach($this->plugins_data as $addon_slug => $data){
			$wc_pbpn_plugin_data = $data; 
			$wc_pbpn_plugin_slug = $addon_slug;
			$required_plugins = $this->extract_required_plugins($wc_pbpn_plugin_data);
			include(WC_PBPN_ADMIN.'views/addons-single.php');
			unset($wc_pbpn_plugin_data);
		}
		include(WC_PBPN_ADMIN.'views/addons-footer.php');
	}
	
	public function get_addon_category(){
		$category = array();
		$category['all']  = __('All',WC_PBPN_TXT);
        $category['active']  = __('Active',WC_PBPN_TXT);
        $category['inactive']  = __('InActive',WC_PBPN_TXT);
	...
	public function get_html_addon_category($cats){
		$output = '<div class="wp-filter"> <ul class="filter-links wc_pbpn_addons_category">';
	...
	public function extract_required_plugins($wc_pbpn_plugin_data){ 
		$plugins = $wc_pbpn_plugin_data['rplugins'];
	...
	public function get_addon_action_link($plugin_slug,$type="active"){
		$is_active = wc_pbpn_check_active_addon($plugin_slug);
		$action = 'wc_pbpn_activate_addon';
		if($type == 'deactivate'){ $action = 'wc_pbpn_deactivate_addon';} 
		$url = admin_url('admin-ajax.php?action='.$action.'&addon_slug='.$plugin_slug);
		$url = wp_nonce_url($url,$action, 'wc_pbpn_security_code');
		return $url;
	}
	...
	public function get_addon_action_button($plugin_slug,$required_plugins){
		...
		$is_active = wc_pbpn_check_active_addon($plugin_slug);
		$extraClass = '';
		$activate_button_text = __('Activate',WC_PBPN_TXT);
		$inactivate_button_text = __('Deactivate',WC_PBPN_TXT);
	...
	public function get_plugins($plugin_folder = '') {
		...
		foreach ( $plugin_files as $plugin_file ) { 
            ...
            $is_active = wc_pbpn_check_active_addon("$plugin_file");
	...
	public function get_plugin_data( $plugin_file, $markup = true, $translate = true ) {
		...
		$plugin_data = get_file_data( $plugin_file, $default_headers, 'wc_pbpn_plugin' );

		if(empty($plugin_data['TextDomain'])){$plugin_data['TextDomain'] = WC_PBPN_TXT;}
	...
	public function get_addon_icon($data,$echo = true){
		$icon = WC_PBPN_IMG.'addon_icon.jpg';
		if(file_exists($data['addon_root'].'icon.png') ){
			$icon = WC_PBPN_ADDON_URL.$data['addon_folder'].'icon.png'; 
		} else if(file_exists($data['addon_root'].'icon.jpg')){
			$icon = WC_PBPN_ADDON_URL.$data['addon_folder'].'icon.jpg';
		} else if(file_exists($data['addon_root'].$data['addon_slug'].'-icon.png')){
			$icon = WC_PBPN_ADDON_URL.$data['addon_folder'].$data['addon_slug'].'-icon.png';
		} else if(file_exists($data['addon_root'].$data['addon_slug'].'-icon.jpg')){
			$icon = WC_PBPN_ADDON_URL.$data['addon_folder'].$data['addon_slug'].'-icon.jpg';
